...
class DistributedKRepeatSampler(Sampler):
    ...
    def __iter__(self):
        while True:
            ...
            # 4. Slice for THIS GPU
            start = self.rank * self.batch_size
            end = start + self.batch_size
            my_indices = shuffled_samples[start:end]

            # 5. Yield indices ONE BY ONE
            # The DataLoader will handle the micro-batching
...
def eval(pipeline, test_dataloader, text_encoders, tokenizers, config, accelerator, global_step, reward_fn, executor, autocast, num_train_timesteps, ema, transformer_trainable_parameters):
    ...
    for test_batch in tqdm(
        ...
        if config.dataset_type == "subject_driven":
            prompts, prompt_metadata, prompt_with_image_paths, ref_images, ref_image_masks = test_batch
        else:
        ...
        rewards = executor.submit(reward_fn, images, prompts, prompt_metadata, ref_images, ref_image_masks, only_strict=False)
        # yield to to make sure reward computation starts
        time.sleep(0)
        rewards, reward_metadata = rewards.result()
        success_signal = reward_metadata.get("success_signal", 1.0)
        
        new_rewards = {}
        
        for key, value in rewards.items():
            if isinstance(value, list) and isinstance(value[0], torch.Tensor):
                # Handles list of [48, 48] tensors
                tensor_val = torch.stack(value).to(accelerator.device).float()
            else:
                # Handles standard list of floats
                tensor_val = torch.as_tensor(value, device=accelerator.device).float()
            
            new_rewards[key] = tensor_val
    ...
    # convert ref_images from list of PIL to list of numpy
    ref_images = [np.array(img).transpose(2,0,1) / 255.0 for img in ref_images]  # HWC -> CHW and normalize to [0,1]
    # convert ref_images to a numpy array to support faster tensor conversion later
    ref_images = np.array(ref_images, dtype=np.float32)
    last_batch_ref_images_gather = accelerator.gather(torch.as_tensor(ref_images, device=accelerator.device)).float().cpu().numpy()
        ...
        with tempfile.TemporaryDirectory() as tmpdir:
            ...
            for idx, index in enumerate(sample_indices):
                ...
                pil_ref = Image.fromarray(
                    (ref_image.transpose(1, 2, 0) * 255).astype(np.float32).astype(np.uint8)  # CHW -> HWC
                )
                pil_ref = pil_ref.resize((config.resolution, config.resolution))
                pil_ref.save(os.path.join(tmpdir, f"{idx}_ref.jpg"))
            ...
            wandb.log(
                {
                    "eval_images": [
                        wandb.Image(
                            ...
                            caption=f"{prompt:.1000} | " + " | ".join(f"{k}: {v.mean():.2f}" for k, v in reward.items()),
                        )
            ...
            if not REF_IMAGES_LOGGED:
                wandb.log({
                "eval_ref_images": [
                        wandb.Image(
                            os.path.join(tmpdir, f"{idx}_ref.jpg"),
                            caption=f"{prompt:.1000} | " + " | ".join(f"{k}: {v.mean():.2f}" for k, v in reward.items()),
                        )
                        for idx, (prompt, reward) in enumerate(zip(sampled_prompts, sampled_rewards))
                    ],
...
def main(_):
    ...
    if "micro_batch_size" in config.sample:
        micro_batch_size = config.sample.micro_batch_size
        micro_batch_size_accumulation_steps = int(config.sample.num_image_per_prompt / (micro_batch_size * config.sample.num_processes))
    else:
        micro_batch_size = None
        micro_batch_size_accumulation_steps = 1
    ...
    accelerator = Accelerator(
        ...
        gradient_accumulation_steps=config.train.gradient_accumulation_steps * num_train_timesteps * micro_batch_size_accumulation_steps,
    )
    if accelerator.is_main_process:
        wandb.init(
            project="DivRL",
    ...
    if config.use_lora:
        ...
        if config.train.lora_path:
            ...
            print(f"Load from lora_path: {config.train.lora_path}")
    ...
    is_filter_zero = config.get("is_filter_zero", False)
    ...
    if "dataset_type" in config and config.dataset_type == "subject_driven":
        ...
        train_dataloader = DataLoader(
            train_dataset,
            batch_size=config.sample.micro_batch_size if micro_batch_size is not None else config.sample.train_batch_size,
            sampler=train_sampler,
            num_workers=0,
            collate_fn=SubjectDrivenDataset.collate_fn,
    ...
    if accelerator.state.deepspeed_plugin:
        accelerator.state.deepspeed_plugin.deepspeed_config['train_micro_batch_size_per_gpu'] = config.sample.micro_batch_size or config.sample.train_batch_size
    # prepare prompt and reward fn
    is_hinge = config.get("is_hinge", False)
    hinge_dict = config.get("hinge_dict", None)
    ...
    if is_deepspeed_zero3_enabled():
        ...
        unset_hf_deepspeed_config()
        reward_fn = getattr(flow_grpo.rewards, 'multi_score')(accelerator.device, config.reward_fn, is_hinge, hinge_dict, semantic_threshold)
        eval_reward_fn = getattr(flow_grpo.rewards, 'multi_score')(accelerator.device, config.reward_fn, is_hinge, hinge_dict, semantic_threshold)
    ...
    logger.info(f"  Actual Sample mini batch size per device = {micro_batch_size if micro_batch_size is not None else config.sample.train_batch_size}")
    logger.info(f"  Train batch size per device = {config.sample.micro_batch_size if micro_batch_size is not None else config.sample.train_batch_size}")
    logger.info(
    ...
    save_dir = config.save_dir + f"_{timestamp}"
    print("Saving checkpoint to:", save_dir)
    if accelerator.is_main_process:
        # wandb.log({"save_dir": save_dir}, step=global_step)
    ...
    while True:
        ...
        for i in tqdm(
            ...
            if config.dataset_type == "subject_driven":
                prompts, prompt_metadata, prompt_with_image_paths, ref_images, ref_image_masks = next(train_iter)
            else:
            ...
            if micro_batch_size is None:
                ...
                timesteps = pipeline.scheduler.timesteps.repeat(
                    micro_batch_size, 1
                )  # (micro_batch_size, num_steps)

            # compute rewards asynchronously
            rewards = executor.submit(reward_fn, images, prompts, prompt_metadata, ref_images, ref_image_masks, only_strict=True)
        ...
        for sample in tqdm(
            ...
            for key, value in rewards.items():
                ...
                sample["rewards"][key] = tensor_val
            
            if "success_signal" in reward_metadata:
                sample["success_signal"] = torch.as_tensor(reward_metadata["success_signal"], device=accelerator.device).float()
                ...
                wandb.log(
                    {
                        "images": [
                            wandb.Image(
                                os.path.join(tmpdir, f"{idx}.jpg"),
                                caption=f"{prompt:.100} | avg: {avg_reward.mean():.2f}",
                            )
        ...
        if config.per_prompt_stat_tracking:
            ...
            if not is_filter_zero:
                advantages = (gathered_rewards['avg'] - gathered_rewards['avg'].mean()) / (gathered_rewards['avg'].std() + 1e-4)
                # ungather advantages; we only need to keep the entries corresponding to the samples on this process
            else:
                non_zero_mask = gathered_rewards['avg'] != 0.0
                cnt_non_zero = non_zero_mask.sum()
                if accelerator.is_main_process:
                    wandb.log({"non_zero_ratio": cnt_non_zero / len(non_zero_mask)}, step=global_step)
                advantages = np.zeros_like(gathered_rewards['avg'])
                advantages[non_zero_mask] = (gathered_rewards['avg'][non_zero_mask] - gathered_rewards['avg'][non_zero_mask].mean()) #  / (gathered_rewards['avg'][non_zero_mask].std() + 1e-4)
                advantages[~non_zero_mask] = -1
        ...
        mask = (samples["advantages"].abs().sum(dim=1) != 0.0)
        ...
        # samples = {
        #     k: v[mask] if k != "success_signal" else v 
        ...
        for inner_epoch in range(config.train.num_inner_epochs):
            ...
            # samples = {
            #     k: v[perm] if k != "success_signal" else v 
            #     for k, v in samples.items()
            # }
            ...
            # samples_batched = {
            #     k: v.reshape(-1, total_batch_size//config.sample.num_batches_per_epoch, *v.shape[1:]) if k != "success_signal" else v
            #     for k, v in samples.items()
            # }
            ...
            for i, sample in tqdm(
                ...
                for j in tqdm(
                    ...
                    with accelerator.accumulate(transformer):
                        ...
                        if "success_signal" in sample.keys():
                            success_signal = sample["success_signal"]
                        else:
                            success_signal = None
                        if success_signal is not None:
                            local_min = torch.min(success_signal)
                            dist.all_reduce(local_min, op=dist.ReduceOp.MIN)
                            global_success = local_min.item()
                        else:
                            global_success = 1.0

                        if global_success < 0.5:
                            loss = loss * 0.0
                            if accelerator.is_main_process:
                                print(f"Skipping update at global step {global_step} due to failed reward computation.")
